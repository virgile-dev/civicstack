...
module.exports = Ractive.extend({
  ...
  validate: function validate(step) {
    ...
    if ((step - 1) == 1) {
      ...
      validateAttribute('github');
      validateAttribute('category');
      if (!app.description[language()]) {
        this.set('errors.description', 'error')
        ok = false;
      } else if (app.description[language()].length > 300) {
        this.set('errors.description', 'error')
        ok = false;
      }
    }
