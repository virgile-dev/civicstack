...
exports.admin = function staff(req, res, next) {
  if (!req.user) return log('User not logged in'), res.status(403);
  if (!req.user.admin()) return log('User is not from staff'), res.status(403);
  next();
}